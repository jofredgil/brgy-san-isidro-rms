...
const DOC_TYPES = [
  ...
  "Certificate of Business Permit/Clearance",
...
const BRANDING = {
  ...
  logo1: "/gakit.jpg", // Left Logo (Republic of the Philippines)
  logo2: "/san isidro logo.jpg", // Right Logo (DILG or Barangay Logo)
  landingBackground: "/bg1.jpg" // Main background image
};
  ...
  useEffect(() => {
    ...
    const unsubUsers = onSnapshot(usersRef, (snap) => {
      ...
      if (fetchedUsers.length === 0) {
        const adminDoc = doc(usersRef, 'admin1');
        setDoc(adminDoc, {
          email: 'admin@sanisidro.gov',
          password: 'admin',
          role: 'admin',
          profile: { name: 'System Administrator', image: '' }
    ...
    const unsubHouseholds = onSnapshot(householdsRef, (snap) => {
    ...
    const unsubOfficials = onSnapshot(officialsRef, (snap) => {
      ...
      if (fetchedOfficials.length === 0) {
        DEFAULT_OFFICIALS.forEach(async (off) => await addDoc(officialsRef, off));
      } else {
...
function LandingScreen({ onLogin, onSignup, households }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (view === 'login') {
      ...
      const success = await onSignup({ 
        email, password, image: profileImage, firstName, middleName, lastName, age, gender, dateOfBirth, placeOfBirth, 
        nationality, religion, address, homeAddress, householdId, isHouseholdHead, contactNumber, occupation, educationalAttainment, isVoter, isPwd, is4ps 
      });
  ...
  return (
    <div 
      className="relative min-h-screen w-full bg-cover bg-center bg-no-repeat font-sans selection:bg-blue-500 selection:text-white flex flex-col"
...
function AdminDashboard({ users, requests, households, officials, onLogout, showToast, currentUser }) {
  ...
  const [resForm, setResForm] = useState({
    email: '', password: '', image: '', firstName: '', middleName: '', lastName: '',
    age: '', gender: 'Male', dateOfBirth: '', placeOfBirth: '', nationality: 'Filipino',
    religion: 'Roman Catholic', address: `Purok ${PUROKS[0]}`, homeAddress: '', householdId: '', isHouseholdHead: 'false', contactNumber: '', 
    occupation: '', educationalAttainment: '', isVoter: 'false', isPwd: 'false', is4ps: 'false', accountStatus: 'Active'
  });
  ...
  const [hhForm, setHhForm] = useState({
    headName: '', address: '', purok: `Purok ${PUROKS[0]}`, is4ps: 'No', status: 'Active'
  });
  ...
  const [offForm, setOffForm] = useState({
    name: '', position: '', yearOfTerm: '', image: ''
  });
  ...
  const stats = useMemo(() => {
    return {
      ...
      male: residents.filter(r => r.profile.gender === 'Male').length,
      female: residents.filter(r => r.profile.gender === 'Female').length,
      minor: residents.filter(r => r.profile.age < 18).length,
      adult: residents.filter(r => r.profile.age >= 18).length,
      senior: residents.filter(r => r.profile.age >= 60).length,
      pwd: residents.filter(r => r.profile.isPwd).length,
      voters: residents.filter(r => r.profile.isVoter).length,
      fourPs: residents.filter(r => r.profile.is4ps).length,
      pendingReqs: requests.filter(r => r.status === 'Pending').length
    };
  }, [residents, requests, households]);

  const handleSaveRequestDetails = async (e) => {
    e.preventDefault();
    setIsSaving(true);
    try {
      const reqDoc = doc(db, 'artifacts', appId, 'public', 'data', 'requests', selectedReq.id);
      await updateDoc(reqDoc, { status: reqStatus, adminNote: adminNote, dateProcessed: new Date().toISOString() });
      setIsReqModalOpen(false);
      showToast("Request updated successfully.");
    } catch (err) { console.error(err); showToast("Failed to update request.", "error"); }
    setIsSaving(false);
  };
  ...
  const handleDeleteRequest = async (reqId) => {
    ...
    } catch (err) { console.error(err); showToast("Failed to delete request.", "error"); }
  };
  ...
  const openAddResident = () => {
    ...
    setResForm({
      email: '', password: '', image: '', firstName: '', middleName: '', lastName: '', age: '', gender: 'Male', dateOfBirth: '', placeOfBirth: '', 
      nationality: 'Filipino', religion: 'Roman Catholic', address: `Purok ${PUROKS[0]}`, homeAddress: '', householdId: '', isHouseholdHead: 'false', contactNumber: '', 
      occupation: '', educationalAttainment: '', isVoter: 'false', isPwd: 'false', is4ps: 'false', accountStatus: 'Active'
    });
  ...
  const openEditResident = (res) => {
    ...
    setResForm({
      email: res.email || '', password: res.password || '', image: res.profile.image || '',
      firstName: res.profile.firstName || '', middleName: res.profile.middleName || '', lastName: res.profile.lastName || '',
      age: res.profile.age || '', gender: res.profile.gender || 'Male', dateOfBirth: res.profile.dateOfBirth || '', placeOfBirth: res.profile.placeOfBirth || '', 
      nationality: res.profile.nationality || 'Filipino', religion: res.profile.religion || 'Roman Catholic', address: res.profile.address || `Purok ${PUROKS[0]}`, homeAddress: res.profile.homeAddress || '', 
      householdId: res.profile.householdId || '', isHouseholdHead: res.profile.householdRole === 'Head' ? 'true' : 'false', contactNumber: res.profile.contactNumber || '', occupation: res.profile.occupation || '', educationalAttainment: res.profile.educationalAttainment || '', 
      isVoter: res.profile.isVoter ? 'true' : 'false', isPwd: res.profile.isPwd ? 'true' : 'false', is4ps: res.profile.is4ps ? 'true' : 'false', accountStatus: res.accountStatus || 'Active',
      civilStatus: res.profile.civilStatus || 'Single'
    });
    setIsResModalOpen(true);
  };
  ...
  const handleSaveResident = async (e) => {
    ...
    if (resModalMode === 'add' && users.find(u => u.email === resForm.email)) {
    ...
    try {
      ...
      const resData = {
        ...
        profile: {
          name: fullName, image: resForm.image, firstName: resForm.firstName, middleName: resForm.middleName, lastName: resForm.lastName,
          age: parseInt(resForm.age) || 0, gender: resForm.gender, address: resForm.address, homeAddress: resForm.homeAddress,
          municipality: 'Gigaquit', householdId: resForm.householdId, householdRole: resForm.isHouseholdHead === 'true' ? 'Head' : 'Member', contactNumber: resForm.contactNumber,
          contactEmail: resForm.email, occupation: resForm.occupation, educationalAttainment: resForm.educationalAttainment,
          isVoter: resForm.isVoter === 'true', isPwd: resForm.isPwd === 'true', is4ps: resForm.is4ps === 'true',
          civilStatus: resForm.civilStatus || 'Single', dateOfBirth: resForm.dateOfBirth, placeOfBirth: resForm.placeOfBirth,
          nationality: resForm.nationality, religion: resForm.religion, zipCode: '8400'
        }
      };

      if (resModalMode === 'add') {
        resData.dateOfRegistration = new Date().toISOString();
        await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'users'), resData);
        showToast("Resident added successfully.");
      } else {
        await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'users', selectedResId), resData);
  ...
  const handleDeleteResident = async (resId) => {
    if(window.confirm("Are you sure you want to permanently delete this resident?")) {
      try {
        await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'users', resId));
        showToast("Resident deleted successfully.");
      } catch (err) { console.error(err); showToast("Failed to delete resident.", "error"); }
    }
  };

  const generateHHNumber = () => {
    if (households.length === 0) return "HH-001";
    const nums = households.map(h => {
      const match = h.hhNumber.match(/\d+/);
  ...
  const openAddHousehold = () => {
    setHhModalMode('add');
    setSelectedHhId(null);
    setHhForm({ headName: '', address: '', purok: `Purok ${PUROKS[0]}`, is4ps: 'No', status: 'Active' });
    setIsHhModalOpen(true);
  };

  const openEditHousehold = (hh) => {
    setHhModalMode('edit');
    setSelectedHhId(hh.id);
    setHhForm({ headName: hh.headName, address: hh.address || '', purok: hh.purok || `Purok ${PUROKS[0]}`, is4ps: hh.is4ps ? 'Yes' : 'No', status: hh.status || 'Active' });
    setIsHhModalOpen(true);
  };
  ...
  const handleSaveHousehold = async (e) => {
    ...
    try {
      const hhData = {
        headName: hhForm.headName.toUpperCase(),
        address: hhForm.address,
        purok: hhForm.purok,
        is4ps: hhForm.is4ps === 'Yes',
        status: hhForm.status,
      };

      if (hhModalMode === 'add') {
        hhData.hhNumber = generateHHNumber();
        hhData.dateRegistered = new Date().toISOString();
        await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'households'), hhData);
        showToast("Household created successfully.");
      } else {
        await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'households', selectedHhId), hhData);
  ...
  const handleDeleteHousehold = async (hhId) => {
    if(window.confirm("Delete this household? Residents inside will lose their household link.")) {
      try {
        await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'households', hhId));
  ...
  const openAddOfficial = () => {
    ...
    setOffForm({ name: '', position: '', yearOfTerm: '', image: '' });
    setIsOffModalOpen(true);
  };

  const openEditOfficial = (off) => {
    setOffModalMode('edit');
    setSelectedOffId(off.id);
    setOffForm({ name: off.name, position: off.position, yearOfTerm: off.yearOfTerm || '', image: off.image || '' });
    setIsOffModalOpen(true);
  };
  ...
  const handleSaveOfficial = async (e) => {
    ...
    try {
      const payload = {
        ...offForm,
        image: offForm.image || `https://api.dicebear.com/7.x/avataaars/svg?seed=${offForm.name}`
      };
      if (offModalMode === 'add') {
        await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'officials'), payload);
        showToast("Official added successfully.");
      } else {
        await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'officials', selectedOffId), payload);
  ...
  const handleDeleteOfficial = async (offId) => {
    if(window.confirm("Are you sure you want to remove this official?")) {
      try {
        await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'officials', offId));
  ...
  const filteredResidents = residents.filter(r => {
    ...
    const purokFilterValue = resPurokFilter === 'All Puroks' ? 'All' : `Purok ${resPurokFilter}`;
    const matchesPurok = resPurokFilter === 'All Puroks' || r.profile.address === purokFilterValue;
    const matchesStatus = resStatusFilter === 'All Status' || r.accountStatus === resStatusFilter;
    
    const matchesTag = resTagFilter === 'All Tags' || 
      (resTagFilter === 'Senior' && r.profile.age >= 60) ||
      (resTagFilter === 'PWD' && r.profile.isPwd) ||
      (resTagFilter === 'Voter' && r.profile.isVoter) ||
      (resTagFilter === '4Ps' && r.profile.is4ps);

    return matchesSearch && matchesGender && matchesPurok && matchesStatus && matchesTag;
  });

  const filteredHouseholds = households.filter(h => 
    h.hhNumber.toLowerCase().includes(hhSearch.toLowerCase()) || 
    h.headName.toLowerCase().includes(hhSearch.toLowerCase())
  ).sort((a,b) => a.hhNumber.localeCompare(b.hhNumber));
  ...
  const filteredVoters = residents.filter(r => {
    const isAdultOrVoter = r.profile.age >= 18 || r.profile.isVoter;
    if (!isAdultOrVoter) return false; 
    const matchesSearch = r.profile.name.toLowerCase().includes(voterSearch.toLowerCase());
    const matchesStatus = voterFilter === 'All' || 
      (voterFilter === 'Registered' && r.profile.isVoter) || 
      (voterFilter === 'Non-Voters' && !r.profile.isVoter);
  ...
  return (
    ...
    >
          ...
          {activeTab === 'dashboard' && (
            <div className="animate-in fade-in duration-300 max-w-7xl mx-auto space-y-6">
              ...
              <div className="grid lg:grid-cols-2 gap-6 mt-6">
                {/* Gender Distribution Chart Panel */}
                <div className="bg-white/90 backdrop-blur-md rounded-xl shadow-sm border border-slate-200 p-6">
                  ...
                  <div className="space-y-6 mb-8">
                    <div className="cursor-pointer group">
                      <div className="flex justify-between items-end mb-2">
                        <span className="text-sm font-bold text-slate-500 flex items-center group-hover:text-blue-500 transition-colors">
                           <span className="text-blue-500 mr-2 text-lg">♂</span> Male
                        </span>
                        <span className="font-extrabold text-slate-800 group-hover:text-blue-600 transition-colors">{stats.male}</span>
                      </div>
                      <div className="w-full bg-slate-100 rounded-full h-2.5">
                        <div className="bg-blue-500 h-2.5 rounded-full shadow-sm" style={{ width: `${(stats.male/stats.population)*100 || 0}%` }}></div>
                      </div>
                    </div>
                    <div className="cursor-pointer group">
                      <div className="flex justify-between items-end mb-2">
                        <span className="text-sm font-bold text-slate-500 flex items-center group-hover:text-pink-500 transition-colors">
                           <span className="text-pink-500 mr-2 text-lg">♀</span> Female
                        </span>
                        <span className="font-extrabold text-slate-800 group-hover:text-pink-600 transition-colors">{stats.female}</span>
                      </div>
                      <div className="w-full bg-slate-100 rounded-full h-2.5">
                        <div className="bg-pink-500 h-2.5 rounded-full shadow-sm" style={{ width: `${(stats.female/stats.population)*100 || 0}%` }}></div>
                      </div>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="border border-slate-100 rounded-xl p-4 bg-slate-50/80 cursor-pointer hover:bg-slate-100 transition-colors">
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Adults</p>
                      <p className="text-2xl font-extrabold text-blue-600">{stats.adult}</p>
                    </div>
                    <div className="border border-slate-100 rounded-xl p-4 bg-slate-50/80 cursor-pointer hover:bg-slate-100 transition-colors">
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Minors</p>
                      <p className="text-2xl font-extrabold text-amber-500">{stats.minor}</p>
                    </div>
                  </div>
                </div>

                {/* Special Groups Panel */}
                <div className="bg-white/90 backdrop-blur-md rounded-xl shadow-sm border border-slate-200 p-6">
                  <div className="flex items-center font-extrabold text-slate-800 mb-6">
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50/80 text-slate-500 text-[10px] uppercase font-extrabold tracking-wider border-b border-slate-200">
                      <th className="p-4 whitespace-nowrap">HH #</th>
                      <th className="p-4">Head of Family</th>
                      <th className="p-4">Address</th>
                      <th className="p-4">Purok/Sitio</th>
                      <th className="p-4 text-center">Members</th>
                      <th className="p-4 text-center">4Ps</th>
                          ...
                          <tr className={`hover:bg-slate-50/80 transition-colors group ${isExpanded ? 'bg-slate-50' : ''}`}>
                            ...
                            <td className="p-4 text-center">
                              <button 
                                ...
                                className="cursor-pointer inline-flex items-center px-3 py-1 bg-blue-50 text-blue-700 text-xs font-extrabold rounded-md hover:bg-blue-100 transition-colors border border-blue-100 shadow-sm"
                              >
                            ...
                            <td className="p-4 text-center space-x-1 whitespace-nowrap">
                              <button onClick={() => openEditHousehold(hh)} className="cursor-pointer p-1.5 bg-white border border-slate-200 text-slate-500 rounded-md hover:text-blue-600 hover:border-blue-200 transition-colors shadow-sm hover:-translate-y-0.5" title="Edit">
                                <Pencil className="w-4 h-4" />
                              </button>
                              <button onClick={() => handleDeleteHousehold(hh.id)} className="cursor-pointer p-1.5 bg-white border border-slate-200 text-slate-500 rounded-md hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors shadow-sm hover:-translate-y-0.5" title="Delete">
              ...
              <div className="p-4 border-b border-slate-200 flex flex-col md:flex-row justify-between items-center bg-white/50 gap-4">
                ...
                <div className="flex flex-wrap md:flex-nowrap items-center gap-2 w-full md:w-auto">
                  <div className="relative flex-1 md:w-48">
                    <Search className="w-4 h-4 absolute left-3 top-2.5 text-slate-400 cursor-text" />
                    <input 
                      type="text" placeholder="Search resident..." 
                  ...
                  {/* Status Tag Filter visible to show if we clicked from Sidebar */}
                    ...
                    {filteredResidents.length === 0 ? (
                      ...
                      return (
                        <tr key={res.id} className="hover:bg-slate-50/80 transition-colors group cursor-pointer">
                          ...
                          <td className="p-4 text-center">
                            <span className={`px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase ${res.profile.gender === 'Female' ? 'bg-pink-100 text-pink-600' : 'bg-blue-100 text-blue-600'}`}>
                              {res.profile.gender}
                            </span>
                          </td>
                          ...
                          <td className="p-4 text-center space-x-1 whitespace-nowrap">
                            <button onClick={() => openEditResident(res)} className="cursor-pointer p-1.5 bg-white border border-slate-200 text-slate-500 rounded-md hover:text-blue-600 hover:border-blue-200 transition-colors shadow-sm hover:-translate-y-0.5" title="Edit">
                              <Pencil className="w-4 h-4" />
                            </button>
                            <button onClick={() => handleDeleteResident(res.id)} className="cursor-pointer p-1.5 bg-white border border-slate-200 text-slate-500 rounded-md hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors shadow-sm hover:-translate-y-0.5" title="Delete">
                              <Trash2 className="w-4 h-4" />
                            </button>
                          </td>
                        </tr>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50/80 text-slate-500 text-[10px] uppercase font-extrabold tracking-wider border-b border-slate-200">
                        ...
                        <th className="p-4 text-center">Gender</th>
                        <th className="p-4">Purok/Sitio</th>
                        <th className="p-4 text-center">Voter Status</th>
                        <th className="p-4">Household Head</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
              ...
              <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-white/50">
                ...
                <div>
                  <select 
                    ...
                    className="cursor-pointer border border-slate-200 rounded-lg px-3 py-1.5 text-sm font-bold text-slate-600 outline-none bg-slate-50 focus:ring-2 focus:ring-blue-500 hover:bg-white transition-colors"
                  >
      ...
      {/* ADMIN MODAL: ADD/EDIT HOUSEHOLD */}
      {isHhModalOpen && (
      ...
      {/* ADMIN POP-UP MODAL FOR EDITING REQUEST */}
            ...
            <form onSubmit={handleSaveRequestDetails} className="p-6 flex-1 overflow-y-auto">
              ...
              <div className="mb-5">
                ...
                <select 
                  ...
                  className="cursor-pointer w-full p-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-bold text-slate-700"
                >
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2 flex justify-between">
                  <span>Message / Note to Resident</span>
                ...
                <textarea 
                  ...
                  rows="3" 
                  className="cursor-text w-full p-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-sm text-slate-700 resize-none" 
                  placeholder="e.g. 'Please bring a valid ID to claim this document.'"
                ></textarea>
              </div>
      ...
      {/* ADMIN POP-UP MODAL FOR ADD/EDIT OFFICIAL */}
      {isOffModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
